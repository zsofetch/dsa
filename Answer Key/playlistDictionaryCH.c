...
// Initialize dictionary (virtual heap)
void initDict(VirtualHeap *VH) {
    int i, Lim, Half;
    Lim = MAX - 1;
    Half = MAX / 2;
...
void displayDict(VirtualHeap VH) {
    ...
    for (trav = 0; trav < LIM; trav++) {
        if (strcmp(VH.H[trav].elem.genre, "Empty") == 0) {
            ...
            printf("\n  %3d.  %30s - %-15s | %-13s", trav + 1,
                   VH.H[trav].elem.songName,
                   VH.H[trav].elem.singer,
                   VH.H[trav].elem.genre);

            cntr = VH.H[trav].next;
            while (cntr != -1) {
                printf("\n        %30s - %-15s | %-13s",
                       VH.H[cntr].elem.songName,
                       VH.H[cntr].elem.singer,
                       VH.H[cntr].elem.genre);
                cntr = VH.H[cntr].next;
            }
        }
        printf("\n\n - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - ");
...
void deleteMember(VirtualHeap *VH, Song song) {
    ...
    if (strcmp(VH->H[index].elem.genre, song.genre) == 0 &&
        strcmp(VH->H[index].elem.songName, song.songName) == 0) {
    ...
    while (curr != -1) {
        if (strcmp(VH->H[curr].elem.songName, song.songName) == 0 &&
            strcmp(VH->H[curr].elem.genre, song.genre) == 0) {
...
int main() {
    ...
    printf("\nThe song \"%s\" %s in my playlist",
           check.songName, (search(VH, check)) ? "exists" : "does not exist");
    printf("\nThe song \"%s\" %s in my playlist",
           temp.songName, (search(VH, temp)) ? "exists" : "does not exist");
